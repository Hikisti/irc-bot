...
class StockCommand:
    ...
    def get_crypto_price(self, crypto_name, currency="usd"):
        ...
        try:
            ...
            change = data[crypto_name].get(f"price_change_24h", 0)  # Use exact field
            change_percent = data[crypto_name].get(f"{currency}_24h_change", 0)
            volume = data[crypto_name].get(f"{currency}_24h_vol", 0) / 1_000_000_000  # Convert to billions

            # Ensure correct color codes
            color = "\x0309" if change >= 0 else "\x0304"  # Bright Green for positive, Bright Red for negative
            
            return f"\x02{crypto_name.capitalize()} {currency.upper()}:\x02 {price:,.2f} {currency.upper()}, today {color}{change:+.2f} ({change_percent:+.2f}%)\x03. Volume {volume:.2f}B."
